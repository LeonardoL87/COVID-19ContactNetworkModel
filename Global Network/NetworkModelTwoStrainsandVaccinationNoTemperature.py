...
def GetCountriinfo(n,days):
    ...
    return EpiData[len(EpiData)-(days+1):len(EpiData)-1]
# -------------------------------------------------------------------------------
# -------------------------------------------------------------------------------
...
# -------------------------------------------------------------------------------
# -------------------------------------------------------------------------------
def T_inv(fileT,time):
    """
    This fntion return the inverse temperature modulator for the infection rate

    Parameters
    ----------
    fileT : str
        rute to the temperature record file.
    time : vector
        time period.
    ...
    -------
    t : time vector 
    Tinv: inverse temperature relation for time t

    """
    TempData = pd.read_csv(open(fileT), sep="\t")
#    Date=pd.to_datetime(TempData['date'])
    T=TempData['mean']
    T=pd.concat([T,T])
    pos=time[0].day+30*time[0].month-30
...
#    beta=bet * I/N
    ...
    # ---------------------Probabilities---------------------------------------
    efracw   = (1.0 - m.exp(-betaw*dt))                     #|exposed prob wild
    efracm   = (1.0 - m.exp(-betam*dt))                     #|exposed prob mutant
    efracVw  = (1.0 - m.exp(-nuV*betaw*dt))                 #|exposed prob vaccinated wild
    efracVm  = (1.0 - m.exp(-nuV*betam*dt))                 #|exposed prob vaccinated mutant
    ...
    vacfrac1 = (1.0 - m.exp(-rho1*deltaV1*dt))              #|vac 1 prob
    vacfrac2 = (1.0 - m.exp(-rho2*(deltaV1+deltaV2)*dt))            #|vac 2 prob
...
def Simulate_Full_network(g,Countries,days,dt,connectivity,ran):
    ...
    for i in ran:
        ...
        print('Simulating for parameters ',i+1,' of ', len(Outputs))
        ...
        for k in np.arange(1,days):
            ...
            for j in Countries['Country']:
                ...
                print('Simuation in node : ',j, ' Iteration: ',i+1, ' of ',len(Outputs),
                      ' Day:',k+1, ' of ',days)
...
FitFullNetwork(Countries,120)

# FitFullNetwork(['Afghanistan','Albania'],180)
